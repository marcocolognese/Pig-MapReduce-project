...
public class BestDay2 extends Configured implements Tool {

	private static final Logger LOG = Logger.getLogger(BestDay2.class);

	public static void main(String[] args) throws Exception {

		int res = ToolRunner.run(new BestDay2(), args);
		System.exit(res);
	}

	@Override
	public int run(String[] args) throws Exception {
		Job job = Job.getInstance(getConf(), "bestday2");
	...
	public static class Map extends Mapper<LongWritable, Text, Text, Text> {
		...
		public void map(LongWritable key, Text lineText, Context context) throws IOException, InterruptedException {
			...
			for (String l : lines) {
				...
				if (!words[14].equals(""))
					context.write(new Text("Year " + words[0]), new Text(words[3] + "," + words[14]));
			}
